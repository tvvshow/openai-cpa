...
def _is_default_cluster_secret(secret: str) -> bool:
    return str(secret or "").strip() in {"", "wenfxl666", "codex2026"}
...
def _validate_cluster_secret(secret: str) -> tuple[bool, str]:
    current_secret = str(getattr(core_engine.cfg, '_c', {}).get("cluster_secret", "codex2026")).strip()
...
async def check_update(current_version: str, token: str = Depends(verify_token)):
    try:
        ...
        web_url = "https://github.com/tvvshow/openai-cpa/releases/latest"
        ...
        async with httpx.AsyncClient(proxy=proxy_url, timeout=15.0) as client:
            ...
            if resp.status_code == 302:
                ...
                download_url = f"https://github.com/tvvshow/openai-cpa/archive/refs/tags/{remote_version}.zip"
            else:
...
def execute_native_update():
    try:
        ...
        if release_response.status_code == 302:
            ...
            zip_url = f"https://github.com/tvvshow/openai-cpa/archive/refs/tags/{latest_tag}.zip"
        else:
